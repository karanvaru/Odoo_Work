...
class PaymentProjection(models.Model): 
    ...
    assigned_to = fields.Many2one('res.users', 'Assigned To', readonly=True,store=True, track_visibility='onchange',  domain=lambda self: [('is_int_user','=',True),("groups_id", "=", self.env.ref( "payment_projection.group_projection_manager").id)])
    ...
    rafc = fields.Monetary('Reserved Amount for Cheques', currency_field='currency_id',readonly=True, compute='compute_cheque_payment',track_visibility='onchange')
...
# ========= Vendor Bills Total==========================Dayan Compute=========================  
    ...
    def compute_vendor_bill_total(self):
        for rec in self:
            if rec.projection_date:
                vendor_bills = rec.env['account.invoice'].search([('type', '=', 'in_invoice'),('state', 'not in', ['paid']),
                                                                  ('effective_date', '=', rec.projection_date)])
                if vendor_bills:
        ...
        for rec in self:
            if rec.projection_date:
                adv_payments = rec.env['advance.payment'].search(
                    [('x_studio_status', 'in', ['submit']),('effective_date', '=', rec.projection_date)])
                if adv_payments:
...
# ========= Expense Total ==========================Dayan Compute=========================  
    ...
    def compute_expense_total(self):
        for rec in self:
            expense_lists = rec.env['hr.expense.sheet'].search([('state','in',['post']),('effective_date', '=', rec.projection_date)])
            if expense_lists:
...
# ========= Reserved Amount for Cheques Total====================Dayan Compute====================
    @api.multi
    ...
    def compute_vendor_bills(self):
        for inv in self:
            if inv.projection_date:
                vend_bills = inv.env['account.invoice'].search([('type','=','in_invoice'),('state','not in',['paid']),('effective_date','=',inv.projection_date)])
                if vend_bills:
                    val=[(5,0,0)]
    ...
    def compute_advance_payments(self):
        for adv in self:
            if adv.projection_date:
                adv_payments = adv.env['advance.payment'].search([('x_studio_status','in',['submit']),('effective_date','=',adv.projection_date)])
                if adv_payments:
                    val=[(5,0,0)]
    ...
    def compute_expense_sheet(self):
        for exp in self:
            if exp.projection_date:
                expense_list = exp.env['hr.expense.sheet'].search([('state','in',['post']),('effective_date','=',exp.projection_date)])
                if expense_list:
                    val=[(5,0,0)]
                    for rec in expense_list:
                        val.append({
                            ...
                            'state': rec.state,
                            'journal_id': rec.account_move_id
                        })
...
class VendorBillsProjectionLine(models.Model):
    ...
    # account_move_id = fields.Many2one('account.invoice', 'Number')
