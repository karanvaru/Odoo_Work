...
class VanillaSeven(models.Model):
    _name = "vanilla.seven"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = " Vanilla Seven"
    ...
    tag_ids = fields.Many2many('seven.vanilla.one.tags', 'seven_vanilla_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('seven.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('seven.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('seven.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('seven.vanilla.sequence')
        res = super(VanillaSeven, self).create(vals)
        return res
...
class SevenVanillaOneTags(models.Model):
    _name = "seven.vanilla.one.tags"

    _description = "Vanilla seven Tags"

    name = fields.Char('Name')

class SevenOnetoManyRelation(models.Model):
    _name = "seven.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('vanilla.seven', string='relation')


class SevenManyToManyRelation(models.Model):
    _name = "seven.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class SevenManyToOneRelation(models.Model):
    _name = "seven.many.to.one.relation"
