...
from odoo.addons.amazon_ept.models.utils import xml2dict


class amazon_process_log_book(models.Model):
    _name = "amazon.process.log.book"
    _description = 'amazon.process.log.book'
    _order = 'id desc'
    ...
    def create_log_based_on_operation(self, jobs, operation):
        ...
        for job in jobs:
            ...
            if job.request_feed_id.feed_result:
                ...
                lines = result.get('AmazonEnvelope', {}).get('Message', {}).get('ProcessingReport',
                                                                                {}).get('Result',
                                                                                        {})
                ...
                for line in lines:
                    ...
                    product = amazon_product_obj.search(
                        [('seller_sku', '=', sku), ('instance_id', '=', job.instance_id.id)])
                    log_type = False
    ...
    def create(self, vals):
        try:
            sequence = self.env.ref("amazon_ept.seq_amazon_file_process_job")
        except:
            sequence = False
...
class amazon_transaction_log(models.Model):
    ...
    _description = 'amazon.transaction.log'
    _rec_name = 'file_name'
    _order = 'id desc'
    ...
    def _checkmodel(self):
        for record in self:
            model = self.env['amazon.transaction.log'].search(
                [('model_id.model', '=', 'shipping.report.request.history'),
    ...
    def process_report(self):
        view = self.env.ref(
            'amazon_fba_connector.amazon_shipping_report_request_history_form_view_ept')
    ...
    operation_type = fields.Selection([('import', 'Import'), ('export', 'Export')]
                                      , string="Operation", related="job_id.operation_type",
    ...
    manually_processed = fields.Boolean("Manually Processed",
                                        help="If This field is True then it will be hidden from "
                                             "mismatch details",
                                        default=False)
    create_date = fields.Datetime("Created Date")
    file_name = fields.Char(string="File Name", related="job_id.filename", store=False,
                            readonly=True)
