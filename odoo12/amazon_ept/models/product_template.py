...
class product_template(models.Model):
    ...
    product_brand_id = fields.Many2one('product.brand.amz.ept', string="Brand",
                                       help='Select a brand for this product.')

    """Commented by Dhruvi [14-11-2018] as variation_theme_id,child_categ_id & 
    amazon_categ_id is made invisible"""

    # @api.onchange("amazon_categ_id", "child_categ_id")
    # def on_change_product_template(self):
    #     if self.variation_theme_id:
    #         if self.variation_theme_id.amazon_categ_id.id not in self.amazon_categ_id.ids +
    #  self.child_categ_id.ids:
    #             self.variation_theme_id = False
    #     if self.child_categ_id and self.amazon_categ_id and self.child_categ_id.parent_id.id !=
    # self.amazon_categ_id.id:
