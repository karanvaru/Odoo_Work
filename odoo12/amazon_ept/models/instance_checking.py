...
import uuid
import base64
from odoo.addons.amazon_ept.models.sale_order import sale_order
from odoo.addons.amazon_ept.models.settlement_report import settlement_report_ept
from odoo.exceptions import ValidationError
from odoo.addons.amazon_ept.models.enviroment import verification_url
from odoo.addons.base.module.module import Module
from odoo import api, fields, models, modules, tools, _

__metaclass__ = type

_app_name = 'amazon_ept'
            ...
            if record.name == _app_name:
                ...
                try:
                    ...
                    raise ValidationError(
                        "Problem with auto invoice workflow module. "
                        "Please find latest module from emiprotechnologies and upgrade it.")
                auto_invoice_workflow_ept = self.env['ir.module.module'].search(
...
class instance_checking(sale_order):

    def import_sales_order(self, seller, marketplaceids=[], end_date=False, start_date=False):
        try:
            ept_key = self.env['ept.app.key.data'].search([('app', '=', 'amazon_ept')])
            if not ept_key:
                ept_key = self.env['ept.app.key.data'].search([('app', '=', _app_name)])
        except Exception as e:
            raise ValidationError(
                "Problem with Auto Workflow Module.\n"
        ...
        if len(ept_key) == 1:
            ...
            if check:
                return super(instance_checking, self).import_sales_order(seller=seller,
    ...
    def import_sales_order_by_flat_report(self, seller, marketplaceids=[], start_date=False,
                                          end_date=False, status=('Unshipped', 'PartiallyShipped')):
        try:
            ept_key = self.env['ept.app.key.data'].search([('app', '=', 'amazon_ept')])
            if not ept_key:
                ept_key = self.env['ept.app.key.data'].search([('app', '=', _app_name)])
        ...
        if len(ept_key) == 1:
            check = decode_client_key(ept_key, self._cr.dbname, _app_name)
            if check:
                return super(instance_checking, self).import_sales_order_by_flat_report(
                    seller=seller, marketplaceids=marketplaceids, start_date=start_date,
    ...
    def import_sales_order_by_xml_report(self, seller, marketplaceids=[], start_date=False,
                                         end_date=False):
        try:
            ept_key = self.env['ept.app.key.data'].search([('app', '=', 'amazon_ept')])
            if not ept_key:
        ...
        if len(ept_key) == 1:
            check = decode_client_key(ept_key, self._cr.dbname, _app_name)
            if check:
                return super(instance_checking, self).import_sales_order_by_xml_report(
                    seller=seller, marketplaceids=marketplaceids, start_date=start_date,
                    end_date=end_date)
            else:
                raise ValidationError("Something bad happened with your app licence.")
...
def decode_key(key):
    ...
    if md5[4:random_number + 9] == key[len(date_time_str) + 1:len(
            date_time_str) + 1 + random_number + 5]:
        is_valid = True

    processed_str_len = len(to_md5) + len(md5[4:random_number + 9])
    key_part = key[processed_str_len:]

    if hashlib.md5("ept_permanent".encode(encoding='utf_8', errors='strict')).hexdigest()[
       4:random_number + 9] == key_part:
        is_permanent = True
    else:
        expiry_date = False
        count = 0
...
def decode_client_key(ept_key, db_name, app_name):
    ...
    if not ept_key.flag_date or (
            current_date - datetime.strptime(ept_key.flag_date, "%Y-%m-%d %H:%M:%S")).days >= 30:
        headers = {'User-Agent': 'ept/1.0'}
        session = requests.Session()
        mac = get_mac()
        from odoo.addons.amazon_ept.models.enviroment import verification_url
        from odoo.addons.amazon_ept.models.enviroment import erp_verification_url
        try:
            post_url = "%s%s/%s/%s/%s" % (erp_verification_url, key, mac, db_name, app_name)
            response = session.post("%s?rdte=%s" % (verification_url, post_url), headers=headers,
                                    data={'rdte': post_url})
            if not response.status_code == 200:
                ...
                if not response.status_code == 200:
                    return False
            ept_key.write({'flag_date': datetime.now()})
        except:
            pass
    ...
    try:
        mac_pass = False
        mac = get_mac()
        string_to_append = hashlib.md5(
            mac.encode(encoding='utf_8', errors='strict') + db_name.encode(encoding='utf_8',
        ...
        if mac_pass and child_pass:
            if type(child_pass) == type(True):
                ...
                if ex_date < current_date.date():
                    raise ValidationError(
                        _("Your licence has been expired. Please renew it to continue...."))
                return True
        else:
            return False
    except ValidationError:
        raise ValidationError(_("Your licence has been expired. Please renew it to continue...."))
    except:
        return False
