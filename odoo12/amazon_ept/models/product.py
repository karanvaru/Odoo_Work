...
class product_product(models.Model):
    ...
    variation_data = fields.Selection(
        [('parent', 'Parent'), ('child', 'Child'), ('collection-parent', 'collection-parent'),
         ('variation-parent', 'variation-parent')], string="Variation Data", default=False)

    @api.one
    def _get_amazon_variant_count(self):
        self.amazon_variant_count = len(self.amazon_product_ids)

    amazon_product_ids = fields.One2many('amazon.product.ept', 'product_id', 'Amazon Products',
                                         required=True)
    amazon_variant_count = fields.Integer(string='# of Amazon Products',
                                          compute="_get_amazon_variant_count")
    is_amazon_virtual_variant = fields.Boolean("Is Amazon Virtual Variant", default=False,
                                               copy=False)

    @api.multi
    def action_view_amazon_products(self):
        self.ensure_one()
        res = self.env.ref('amazon_ept.action_amazon_product_ept')
...
class amazon_product_ept(models.Model):
    ...
    _description = 'amazon.product.ept'
    _inherits = {'product.product': 'product_id'}

    """Commented by Dhruvi [12-11-2018] as amazon_browse_node_id,amazon_browse_node_id2 
        field is made invisible"""

    # @api.onchange('amazon_browse_node_id', 'amazon_browse_node_id2')
    # def onchange_amazon_browse_node_id(self):
    #     res = {}
    #     domain = {'amazon_browse_node_id': [('country_id', '=', self.instance_id.country_id.id),
    #                                         ('is_root_category', '=', False)],
    #               'amazon_browse_node_id2': [('country_id', '=', self.instance_id.country_id.id),
    #                                          ('is_root_category', '=', False)]}
    #     res['domain'] = domain
    #     return res
    ...
    def _calc_seller(self):
        for product in self:
            ...
            if seller_info_id:
                product.merchant_catalog_number = seller_info_id.product_code and \
                                                  seller_info_id.product_code or ''
    ...
    def _get_main_product_supplier(self, product):
        sellers = [(seller_info.sequence, seller_info)
                   ...
                   if seller_info and isinstance(seller_info.sequence, (int))]  # int, long
    ...
    def get_log_count(self):
        ...
        for product in self:
            records = amazon_transaction_log_obj.search(
                [('model_id', '=', model_id), ('res_id', '=', product.id),
                 ('job_id.request_feed_id', '!=', False)])
    ...
    def get_variants(self):
        for record in self:
            ...
            variants = self.search(
                [('product_id', 'in', product_ids), ('instance_id', '=', record.instance_id.id),
    ...
    liquid_uom = fields.Selection(
        [('cubic-cm', 'Cubic-Cm'), ('cubic-ft', 'Cubic-Ft'), ('cubic-in', 'Cubic-In'),
         ('cubic-m', 'Cubic-mM'),
         ('cubic-yd', 'Cubic-Yd'), ('cup', 'Cup'), ('fluid-oz', 'Fluid-Oz'), ('gallon', 'Gallon'),
         ('liter', 'Liter'), ('milliliter', 'Milliliter'), ('ounce', 'Ounce'), ('pint', 'Pint'),
         ('quart', 'Quart'), ('liters', 'Liters'), ('deciliters', 'Deciliters'),
         ('centiliters', 'Centiliters'),
         ('milliliters', 'Milliliters'), ('microliters', 'Microliters'),
         ('nanoliters', 'Nanoliters'),
         ('picoliters', 'Picoliters')
         ])
    """Commented delivery_channel field by Dhruvi [12-11-2018]"""
    # delivery_channel=fields.Selection([('in_store','In Store'),('direct_ship','Direct Ship')],
    # default='direct_ship')
    ...
    promotion_keywords_ids = fields.Many2many("amazon.promotion.keyword.ept",
                                              'amazon_promotion_keyword_rel', 'product_id',
                                              'promotion_id', string="PromotionKeywords")

    """fields used_for_ids,target_audience_ids,subject_content_ids,other_attribute_ids,
        platinum_keyword_ids
        Commented by Dhruvi [12-11-2018]"""
    # used_for_ids = fields.Many2many("amazon.product.used.for", 'amazon_product_used_for_rel',
    #                                 'product_id', 'used_for_id', string="Used For")
    # target_audience_ids = fields.Many2many("amazon.target.audience", "amazon_target_audience_rel",
    #                                        "product_id", "target_id", string="Target Audience")
    # subject_content_ids = fields.Many2many("amazon.subject.content", "amazon_subject_content_rel",
    #                                        "product_id", "content_id", string="Subject Content")
    # other_attribute_ids = fields.Many2many("amazon.other.item.attributes",
    #                                        "amazon_other_item_attribute_rel", "product_id",
    #                                        "attribute_id", string="Other Attributes")
    # platinum_keyword_ids = fields.Many2many("amazon.platinum.keyword",
    #   "amazon_platinum_keyword_rel","product_id","platinum_keyword_id",string="Platinum Keywords")
    ...
    child_variant_ids = fields.One2many("amazon.product.ept", string="Child Variants",
                                        compute="get_variants")
    ...
    out_of_stock_website_message = fields.Selection(
    ...
    child_display_per_page = fields.Selection([('independently_displayable', 'Independently'), (
        'display_only_on_parent', 'Display Only On Parent')], default='display_only_on_parent')
    ...
    prep_instruction_ids = fields.Many2many("prep.instruction", "amazon_prep_instruction_rel",
                                            "amazon_product_id", "prep_instruction_id",
                                            string="Prep Instruction",
                                            help="Amazon FBA: Prep Instruction")
    barcode_instruction = fields.Selection([('AMAZON', 'Amazon'), ('SELLER', 'Seller')],
                                           string="Barcode Instruction",
                                           help="Amazon FBA: Barcode Instruction")
    quantity_in_case = fields.Float(string="Quantity In Case", default=0.0,
                                    help="Amazon FBA: Quantity In Case")
    ...
    #         mws_obj = Feeds(access_key=instance.access_key and str(instance.access_key) or False,
    ...
    #                         region=instance.country_id.amazon_marketplace_code or
    #                                instance.country_id.code,
    #                         auth_token=instance.auth_token and str(instance.auth_token) or False,
    #                         proxies=proxy_data)
    #
    #         results = mws_obj.submit_feed(data.encode('utf-8'), '_POST_INVENTORY_AVAILABILITY_DATA_',
    ...
    #         amazon_products.write(
    #             {'fulfillment_by': 'MFN', 'last_feed_submission_id': last_feed_submission_id})
    #         return True
    #
    #     @api.multi
    ...
    #         last_feed_submission_id = self.switch_feed_submit(instance, data)
    #         amazon_products.write(
    #             {'fulfillment_by': 'AFN', 'last_feed_submission_id': last_feed_submission_id})
    #         return True
    ...
    def get_product_prep_instructions(self, instance, amazon_products):
        ...
        proxy_data = instance.seller_id.get_proxy_server()
        ship_to_country_code = instance.country_id.amazon_marketplace_code or \
                               instance.country_id.code
        seller_sku_list = []
        ...
        kwargs = {'merchant_id': instance.merchant_id and str(instance.merchant_id) or False,
                  ...
                  'emipro_api': 'get_product_prep_instructions',
                  'dbuuid': dbuuid,
        ...
        for sku_pre_instruction in sku_pre_instructions:
            ...
            amazon_product_record = amazon_product_ept_obj.search(
            ...
            if amazon_product_record:
                ...
                prep_instructions = prep_instruction_obj.search([('name', 'in', [
                    prep_instruction_value_dict.get("value", "") for prep_instruction_value_dict in
                    prep_instruction_value_list])]) or []
                amazon_product_record.write({'barcode_instruction': barcode_instruction,
                                             'prep_instruction_ids': [(6, 0,
                                                                       prep_instructions and
                                                                       prep_instructions.ids)]})
        return True
    ...
    amazon_browse_node_id = fields.Many2one('amazon.browse.node.ept', string='Browse Node',
                                            copy=False)
    amazon_browse_node_id2 = fields.Many2one('amazon.browse.node.ept', string='Browse Node2',
                                             copy=False)
    ...
    package_weight = fields.Float("Package Weight", help="Weight of the package",
    ...
    shipping_weight = fields.Float("Shipping Weight",
                                   help="Weight of the product when packaged to ship",
                                   digits=dp.get_precision("Stock Weight"))
    shipping_weight_uom = fields.Selection(
        [('GR', 'GR'), ('KG', 'KG'), ('OZ', 'OZ'), ('LB', 'LB'), ('MG', 'MG')], default='KG',
        string="Shipping Weight Uom")
    max_order_quantity = fields.Integer("Max Order Quantity",
                                        help="Maximum quantity of the product that a "
                                             "customer can order")
    is_gift_wrap_available = fields.Boolean("Is Gift Wrap Available ?",
                                            help="Indicates whether gift wrapping is available "
                                                 "for the product")
    is_gift_message_available = fields.Boolean("Is Gift Message Available ?",
                                               help="Indicates whether gift messaging is available "
                                                    "for the product")
    is_discontinued_by_manufacturer = fields.Boolean("Is Discontinued By Manufacturer ?",
                                                     help="Indicates that the manufacturer has "
                                                          "stopped making the item")
    max_aggregate_ship_quantity = fields.Integer("Max Aggregate Ship Quantity",
                                                 help="The maximum number of the same item that "
                                                      "can be shipped in the same package")
    standard_product_id_type = fields.Selection(
        [('EAN', 'EAN'), ('ISBN', 'ISBN'), ('UPC', 'UPC'), ('ASIN', 'ASIN'), ('GCID', 'GCID'),
         ('GTIN', 'GTIN'), ('PZN', 'PZN')], string="Standard Product ID", default='ASIN',
        required=True)
    pzn_number = fields.Char("PZN Number")
    tax_code_id = fields.Many2one('amazon.tax.code.ept', string="Product Tax Code", copy=False)
    item_package_qty = fields.Integer(string="Item Package Quantity", help="Number of the same "
...
a case of 10 packages of socks, ItemPackageQuantity would be 10.", default=1)
    ...
    """Field serial_number_required commented by Dhruvi [12-11-2018]"""
    # serial_number_required=fields.Boolean("Serial Number Required",help="Indicates whether the
    # product must have a serial number")
    legal_disclaimer = fields.Text(string="Legal Disclaimer",
                                   help="Any legal disclaimer needed with the product")
    mfr_part_number = fields.Char(string="Mfr Part Number",
                                  help="Part number provided by the original manufacturer")
    ...
    manufacturer = fields.Char(string="Manufacturer",
                               related='product_id.product_tmpl_id.product_brand_id.partner_id.name',
                               readonly=True)
    ...
    target_audience = fields.Selection(
        [('male', 'Male'), ('female', 'Female'), ('unisex', 'Unisex')], string="Target Audience",
        default="unisex")
    ...
    """Fields purchase_off_amazon_channel,purchase_on_amazon_channel,prop
        Commented by Dhruvi [12-11-2018]"""
    # purchase_off_amazon_channel = fields.Selection(
    #     [('advertise', 'advertise'), ('exclude', 'Exclude')], string="OffAmazonChannel")
    # purchase_on_amazon_channel = fields.Selection(
    #     [('sell', 'Sell'), ('advertise', 'Advertise'), ('exclude', 'Exclude')],
    #     string="OnAmazonChannel")
    # prop = fields.Boolean("Prop65")
    # cspia_warning_ids = fields.Many2many('amazon.cspia.warning.ept', 'amazon_cspia_warning_rel',
    #                                      'product_id', 'warning_id', string="CPSIAWarning")
    # cpsia_warning_description = fields.Text("CPSIAWarningDescription")
    # tsd_age_warning = fields.Selection(
    #     [('not_suitable_under_36_months', 'not_suitable_under_36_months'),
    ...
    # tsd_warning_ids = fields.Many2many('amazon.tsd.warning.ept', 'amazon_tsd_warning_rel',
    #                                    'product_id', 'warning_id', string="TSDWarning")
    # tsd_language_ids = fields.Many2many('amazon.tsd.language.ept','amazon_tsd_language_rel',
    # 'product_id','language_id',string="TSDLanguage")
    ...
    payment_option_ids = fields.Many2many('amazon.payment.type.option.ept',
                                          'amazon_payment_type_option_rel', 'product_id',
                                          'payment_id', string="Other Payment Option")
    shipped_by_freight = fields.Boolean("Shipped By Freight")
    registerd_parameter = fields.Selection(
        [("PrivateLabel", 'PrivateLabel'), ('Specialized', 'Specialized'),
         ('NonConsumer', 'NonConsumer'), ('PreConfigured', 'PreConfigured')],
        string="RegisteredParameter")
    ...
    amazon_price_ept = fields.Float("Amazon Price", compute="get_amazon_price",
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        if name:
            ...
            if operator in positive_operators:
                recs = self.search(
                    ['|', ('seller_sku', '=', name), ('default_code', '=', name)] + args,
                    limit=limit)
                if not recs:
            ...
            if not recs and operator not in expression.NEGATIVE_TERM_OPERATORS:
                ...
                # would be abysmal on a database with thousands of matching products, due to the
                # huge merge+unique needed for the OR operator (and given the fact that the 'name'
                # lookup results come from the ir.translation table Performing a quick memory merge
                # of recs in Python will give much better performance
                recs = self.search(
                    args + ['|', ('seller_sku', operator, name), ('default_code', operator, name)],
                    limit=limit)
                if not limit or len(recs) < limit:
                    # we may underrun the limit because of dupes in the results, that's fine
                    limit2 = (limit - len(recs)) if limit else False
                    recs += self.search(
                        args + [('name', operator, name), ('id', 'not in', recs.ids)], limit=limit2)
            elif not recs and operator in expression.NEGATIVE_TERM_OPERATORS:
                recs = self.search(args + ['&', '|', ('seller_sku', operator, name),
            ...
            if not recs and operator in positive_operators:
                ...
                if res:
                    recs = self.search(['|', ('seller_sku', '=', res.group(2)),
    ...
    def export_product_amazon(self, instance, amazon_products):
        ...
        kwargs = {'merchant_id': instance.merchant_id and str(instance.merchant_id) or False,
                  ...
                  'app_name': 'amazon_ept',
                  'account_token': account.account_token,
                  'emipro_api': 'export_product_amazon',
                  'dbuuid': dbuuid,
                  'amazon_marketplace_code': instance.country_id.amazon_marketplace_code or
                                             instance.country_id.code,
                  'proxies': proxy_data,
        ...
        if data:
            kwargs.update({'data': data,
                           'emipro_api': 'export_product_relationship_amazon', })
        ...
        for amazon_product in amazon_products:
            ...
            for variant in amazon_product.child_variant_ids:
                data = "%s %s" % (data, """<Relation>
                    ...
                    variant.seller_sku, amazon_product.child_display_per_page,
                    amazon_product.relation_type))
            message = message + 1
        if data:
        ...
        for amazon_product in self:
            ...
            for child_product in amazon_product.child_variant_ids:
                for image_obj in child_product.product_id.ept_image_ids:
                    message_information = self.create_image_dict(child_product, image_obj,
                                                                 message_information, message_id)
                    message_id = message_id + 1
    ...
    def update_price_dict(self, instance, amazon_product, message_information, message_id):
        ...
        # price = instance.pricelist_id.get_product_price(amazon_product.product_id,1.0,
        # partner=False,uom_id=amazon_product.product_id.uom_id.id)
        price = pricelist_obj.get_product_price_ept(amazon_product.product_id,
                                                    instance.pricelist_id.id, partner=False)
        price = price and round(price, 2) or 0.0
        seller_sku = cgi.escape(amazon_product.seller_sku)
    ...
    def update_price(self, instance):
        ...
        for amazon_product in self:
            message_information = self.update_price_dict(instance, amazon_product,
                                                         message_information, message_id)
            message_id = message_id + 1
            for child_product in amazon_product.child_variant_ids:
                message_information = self.update_price_dict(instance, child_product,
                                                             message_information, message_id)
                message_id = message_id + 1
    ...
    def get_bullet_points(self, product):
        ...
        for bullet in product.bullet_point_ids:
            bullet_point = """<BulletPoint>%s</BulletPoint>""" % (cgi.escape(bullet.name))
    ...
    def get_search_terms(self, product):
        ...
        for search_term in product.search_term_ids:
            search_term = """<SearchTerms>%s</SearchTerms>""" % (cgi.escape(search_term.name))
            search_terms = "%s %s" % (search_terms, search_term)
        return search_terms

    """Commented by Dhruvi [12-11-20018]
        As these fields are commented"""
    ...
    #         pla_key = """<PlatinumKeywords>%s</PlatinumKeywords>""" % (
    #         cgi.escape(platinum_keyword.name).encode("utf-8"))
    ...
    #         us_f = """<PlatinumKeywords>%s</PlatinumKeywords>""" % (
    #         cgi.escape(used_for.name).encode("utf-8"))
    ...
    #         other_att = """<OtherItemAttributes>%s</OtherItemAttributes>""" % (
    #         cgi.escape(other_attribute.name).encode("utf-8"))
    ...
    #         targent_audi = """<TargetAudience>%s</TargetAudience>""" % (
    #         cgi.escape(target_audience.name).encode("utf-8"))
    ...
    #         subject_con = """<SubjectContent>%s</SubjectContent>""" % (
    #         cgi.escape(subject_content.name).encode("utf-8"))
    ...
    def get_promotion_keywords(self, product):
        ...
        for promotion_keyword in product.promotion_keywords_ids:
            promotion_key = """<PromotionKeywords>%s</PromotionKeywords>""" % (
                cgi.escape(promotion_keyword.name).encode("utf-8"))
            promotion_keywords = "%s %s" % (promotion_keyword, promotion_key)
    ...
    def get_lanuch_date(self, product):
        launch_date = product.launch_date and datetime.date.strftime(
            datetime.datetime.strptime(product.launch_date, "%Y-%m-%d %H:%M:%S"),
    ...
    def get_discontinue_date(self, product):
        discontinue_date = product.discontinue_date and datetime.date.strftime(
            datetime.datetime.strptime(product.discontinue_date, "%Y-%m-%d %H:%M:%S"),
            "%Y-%m-%dT%H:%M:%S") or False
    ...
    def get_release_date(self, product):
        release_date = product.release_date and datetime.date.strftime(
            datetime.datetime.strptime(product.release_date, "%Y-%m-%d %H:%M:%S"),
            "%Y-%m-%dT%H:%M:%S") or False
    ...
    def get_description_data(self, product):
        ...
        if product.title:
            ...
            data.append("<Title>%s</Title>" % (
                cgi.escape(product.name)))  # .encode("utf-8")
        product.brand and data.append(
            "<Brand>%s</Brand>" % (cgi.escape(product.brand)))  # .encode("utf-8")
        product.designer and data.append(
            "<Designer>%s</Designer>" % (cgi.escape(product.designer)))  # .encode("utf-8")
        description = product.long_description or product.description or False
        description and data.append("<Description>%s</Description>" % (cgi.escape(description)))
        ...
        # product.cpsia_warning_description and data.append(
        #     cgi.escape(product.cpsia_warning_description))
        ...
        product.legal_disclaimer and data.append("<LegalDisclaimer>%s</LegalDisclaimer>" % (
            cgi.escape(product.legal_disclaimer)))  # .encode("utf-8")
        product.manufacturer and data.append(
            "<Manufacturer>%s</>" % (cgi.escape(product.manufacturer)))  # .encode("utf-8")
        product.mfr_part_number and data.append(
            "<MfrPartNumber>%s</MfrPartNumber>" % (product.mfr_part_number))
        product.search_term_ids and data.append(self.get_search_terms(product))
        product.item_type_keyword and data.append(
            "<ItemType>%s</ItemType>" % (product.item_type_keyword))
        """Commenetd by Dhruvi [12-11-2018]"""
        ...
        # commented by Dhruvi [12-11-2018]
        # product.delivery_channel and data.append(
        #     "<DeliveryChannel>%s</DeliveryChannel>" % (product.delivery_channel))
        ...
        #     product.amazon_browse_node_id.ama_category_code))
        # product.amazon_browse_node_id2 and data.append(
        #     "<RecommendedBrowseNode>%s</RecommendedBrowseNode>" % (
        #     product.amazon_browse_node_id2.ama_category_code))
        # product.distribution_designation and data.append(
        #     "<DistributionDesignation>%s</DistributionDesignation>" % (
        #     product.distribution_designation))
        # product.delivery_schedule_group and data.append(
        #     "<DeliveryScheduleGroupID>%s</DeliveryScheduleGroupID>" % (
        #     product.delivery_schedule_group))
        ...
        if product.max_aggregate_ship_quantity > 0:
            data.append("<MaxAggregateShipQuantity>%s</MaxAggregateShipQuantity>" % (
                product.max_aggregate_ship_quantity))
        ...
        product.merchant_shipping_group_name and data.append(
            "<MerchantShippingGroupName>%s</MerchantShippingGroupName>" % (
                product.merchant_shipping_group_name))
    ...
    """Commented by Dhruvi [13-11-2018] as field browse_exclusion,recommendation_exclusion are made invisible """
    ...
    #         discovery = "%s %s" % (
    #         discovery, "<RecommendationExclusion>true</RecommendationExclusion>")
    ...
    def get_payment_options(self, product):
        ...
        for option in product.payment_option_ids:
            payment_option = "<OptionalPaymentTypeExclusion>%s</OptionalPaymentTypeExclusion>" % (
                option.name)
    ...
    def get_tax_code(self, product):
        if product.tax_code_id:
            ...
            return """<ProductTaxCode>%s</ProductTaxCode>""" % (
                product.instance_id.default_amazon_tax_code_id.name)
        else:
            return False
    ...
    def get_rebate(self, product):
        rebate_start_date = product.rebate_start_date and datetime.date.strftime(
            datetime.datetime.strptime(product.rebate_start_date, "%Y-%m-%d %H:%M:%S"),
            "%Y-%m-%dT%H:%M:%S") or False
        rebate_end_date = product.rebate_end_date and datetime.date.strftime(
            datetime.datetime.strptime(product.rebate_end_date, "%Y-%m-%d %H:%M:%S"),
            "%Y-%m-%dT%H:%M:%S") or False
        ...
        """ % (
            rebate_start_date, rebate_end_date, cgi.escape(product.rebate_message),
            product.rebate_name)
    ...
    def get_message(self, message_id, product):
        ...
        """Commenetd by Dhruvi [13-11-2018]"""
        # if product.purchase_off_amazon_channel:
        #     message = "%s %s" % (message, "<OffAmazonChannel>%s</OffAmazonChannel>" % (
        #     product.purchase_off_amazon_channel))
        # if product.purchase_on_amazon_channel:
        #     message = "%s %s" % (
        #     message, "<OnAmazonChannel>%s</OnAmazonChannel>" % (product.purchase_on_amazon_channel))
        ...
        if product.liquid_volume:
            message = "%s %s" % (message, "<LiquidVolume unitOfMeasure='%s'>%s</LiquidVolume>" % (
                product.liquid_uom, product.liquid_volume))
        description_data = self.get_description_data(product)
        message = "%s %s" % (message, description_data)
        if product.amazon_vendor_cost:
            message = "%s <Amazon-Vendor-Only>%s</Amazon-Vendor-Only>" % (
                message, product.amazon_vendor_cost)
        ...
        if product.purchasing_category:
            amazon_only = "%s %s" % (amazon_only, "<PurchasingCategory>%s</PurchasingCategory>" % (
                product.purchasing_category))
        if product.purchasing_sub_category:
            amazon_only = "%s %s" % (amazon_only,
                                     "<PurchasingSubCategory>%s</PurchasingSubCategory>" % (
                                         product.purchasing_sub_category))
        if product.packaging_type:
            amazon_only = "%s %s" % (
                amazon_only, "<PackagingType>%s</PackagingType>" % (product.packaging_type))
        if product.underlying_availability:
            amazon_only = "%s %s" % (amazon_only,
                                     "<UnderlyingAvailability>%s</UnderlyingAvailability>" % (
                                         product.underlying_availability))
        if product.replenishment_category:
            amazon_only = "%s %s" % (amazon_only,
                                     "<ReplenishmentCategory>%s</ReplenishmentCategory>" % (
                                         product.replenishment_category))
        if product.drop_ship_status:
            amazon_only = "%s %s" % (
                amazon_only, "<DropShipStatus>%s</DropShipStatus>" % (product.drop_ship_status))
        if product.out_of_stock_website_message:
            amazon_only = "%s %s" % (amazon_only,
                                     "<OutOfStockWebsiteMessage>%s</OutOfStockWebsiteMessage>" % (
                                         product.out_of_stock_website_message))
        ...
        if product.registerd_parameter:
            message = "%s %s" % (
                message,
                "<RegisteredParameter>%s</RegisteredParameter>" % product.registerd_parameter)
        message = "%s </Product>" % (message)
        return "<Message>%s</Message>" % (message)

    """Commented by Dhruvi [14-11-2018] as amazon_categ is made invisible"""
    ...
    #                 count = 1
    #                 for value in attribute_line.value_ids:
    #                     attribute_name = "%s_%s_%s" % (
    #                     attribute_line.uom_type, attribute_line.attribute_id.name, count)
    #                     dict_categ_sequence.update({attribute_name: attribute_line.value_id.name})
    #                     count = count + 1
    #             else:
    #                 attribute_name = "%s_%s" % (
    #                 attribute_line.uom_type, attribute_line.attribute_id.name)
    #                 dict_categ_sequence.update({attribute_name: attribute_line.value_id.name})
    #     try:
    #         category_structure = category_structure % DataDict(dict_categ_sequence)
    #         from lxml import etree
    #         doc = etree.XML(category_structure)
    #         elements = doc.xpath('//%s' % (
    #         categ_dict.get(product.amazon_categ_id.name, product.amazon_categ_id.name)))
    ...
    #         if category_structure and not isinstance(category_structure, str):
    #             category_structure = category_structure.decode()
    #     except:
    ...
    @api.multi
    def prepare_export_stock_level_dict(self, amazon_product, instance, message_information,
                                        message_id):
        stock = 0.00
        asin_qty = 1.00
        seller_sku = cgi.escape(amazon_product.seller_sku)
        ...
        if instance.stock_field:
            stock = prod_obj.get_stock_ept(amazon_product.product_id, instance.warehouse_id.id,
        ...
        if amazon_product.allow_package_qty:
            ...
            if stock < 1.00:
                stock = 0.00
        stock = int(stock)
        message_information += """<Message><MessageID>%s</MessageID><OperationType>Update</OperationType><Inventory><SKU>%s</SKU><Quantity>%s</Quantity>""" % (
            message_id, seller_sku, stock)
        """
        Add Parameter in XMl FulfillmentLatency 
        Value will be added from amazon product or from seller configuration
        @author: Deval Jagad (21/12/2019)
        """
        fullfillment_latency = amazon_product.fulfillment_latency or instance.seller_id.fulfillment_latency or 0
        if fullfillment_latency:
            message_information += """<FulfillmentLatency>%s</FulfillmentLatency></Inventory></Message>""" % (
                fullfillment_latency)
        else:
            message_information += """</Inventory></Message>"""
        return message_information
    ...
    def export_stock_levels(self, instance, product_ids=[]):
        ...
        if not product_ids:
            amazon_products = self.search(
                [('exported_to_amazon', '=', True), ('instance_id', '=', instance.id),
                 ('fulfillment_by', '=', 'MFN')])
        else:
        ...
        merchant_string = "<MerchantIdentifier>%s</MerchantIdentifier>" % (instance.merchant_id)
        for amazon_product in amazon_products:
            message_information = self.prepare_export_stock_level_dict(amazon_product, instance,
                                                                       message_information,
                                                                       message_id)
            message_id = message_id + 1
            for child_product in amazon_product.child_variant_ids:
                message_id = message_id + 1
                message_information = self.prepare_export_stock_level_dict(child_product, instance,
                                                                           message_information,
                                                                           message_id)
        if message_information:
