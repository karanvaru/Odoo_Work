...
class website_affiliate(Home):
    ...
    def register(self, **kw):
        ...
        term_condition = request.env['res.config.settings'].sudo(
        ).website_constant().get('term_condition')
        values = {}
    ...
    def register_affiliate(self, **kw):
        ...
        if aff_request and kw.get('confirm_password') == kw.get('password') and aff_request.signup_token == kw.get('token'):
            template_user_id = literal_eval(request.env['ir.config_parameter'].sudo(
            ...
            try:
                ...
                return login_and_redirect(db, data['login'], data['password'], redirect_url='/')
    ...
    def report(self, **kw):
        partner = request.env.user.partner_id
        enable_ppc = request.env['res.config.settings'].sudo(
        ).website_constant().get('enable_ppc')
        currency_id = request.env.user.company_id.currency_id
        visits = request.env['affiliate.visit'].sudo()
        ppc_visit = visits.search([('affiliate_method', '=', 'ppc'), ('affiliate_partner_id',
                                                                      '=', partner.id), '|', ('state', '=', 'invoice'), ('state', '=', 'confirm')])
        pps_visit = visits.search([('affiliate_method', '=', 'pps'), ('affiliate_partner_id',
                                                                      '=', partner.id), '|', ('state', '=', 'invoice'), ('state', '=', 'confirm')])
        values = {
            'pending_amt': partner.pending_amt,
            'approved_amt': partner.approved_amt,
    ...
    def traffic(self, page=1, date_begin=None, date_end=None, **kw):
        ...
        partner = request.env.user.partner_id
        visits = request.env['affiliate.visit'].sudo()
        domain = [('affiliate_partner_id', '=', partner.id), ('affiliate_method', '=',
                                                              'ppc'), '|', ('state', '=', 'invoice'), ('state', '=', 'confirm')]
    ...
    def aff_order(self, page=1, date_begin=None, date_end=None, **kw):
        ...
        visits = request.env['affiliate.visit'].sudo()
        domain = [('affiliate_partner_id', '=', partner.id), ('affiliate_method', '=',
                                                              'pps'), '|', ('state', '=', 'invoice'), ('state', '=', 'confirm')]
        if date_begin and date_end:
            domain += [('create_date', '>', date_begin), ('create_date', '<=', date_end)]
        traffic_count = visits.search_count(domain)
        pager = request.website.pager(
    ...
    def payment(self, page=1, date_begin=None, date_end=None, **kw):
        values = {}
        partner = request.env.user.partner_id
        invoices = request.env['account.invoice']
        domain = [('partner_id', '=', partner.id), ('state', '=', 'paid')]
        if date_begin and date_end:
            domain += [('create_date', '>', date_begin), ('create_date', '<=', date_end)]
    ...
    def aff_invoice_form(self, invoice=None, **kw):
        inv = request.env['account.invoice'].sudo().browse([invoice])
    ...
    def search_product(self, **kw):
        ...
        if kw.get('name'):
            ...
            domain += [
                ('website_published', '=', True), '|', '|', '|', ('name', 'ilike',
                                                                  kw.get('name')), ('description', 'ilike', kw.get('name')),
    ...
    def tool_banner(self, **kw):
        partner = request.env.user.partner_id
        banner_image_ids = request.env['affiliate.image'].sudo().search(
            [('image_active', '=', True)])
