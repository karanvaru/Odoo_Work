...
class RdpDx(models.Model):
    ...
    dx_cancel_ids = fields.One2many('cancel.reason', 'cancel_id', string="Cancel Ids",
    ...
    five_why_count = fields.Integer("Five Why Count", compute="five_why_ticket_count")

    @api.multi
    def five_why_ticket_count(self):
        count_values = self.env['five.why'].search_count([('dx_five_why_id', '=', self.id)])
        self.five_why_count = count_values
    ...
    def action_button(self):
        return {
            ...
            'res_model': 'five.why',
            'context' : {
                'default_dx_five_why_id' : self.id
            },
            'domain': [('dx_five_why_id', '=', self.id)],
...
class CancelReason(models.Model):
    ...
    cancel_id = fields.Many2one('rdp.dx', string='Cancel')
    reason = fields.Text(string='Reason', required=True)
    created_by = fields.Char('Created By')
    created_date = fields.Datetime(string='Created Date', default=datetime.today())

class RdpDxFiveWhy(models.Model):
    _inherit = 'five.why'

    dx_five_why_id = fields.Many2one('rdp.dx', 'Dx')
