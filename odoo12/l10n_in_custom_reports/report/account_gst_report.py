...
class L10nInReportAccount(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        gst_return_type = options.get('gst_return_type')
        gst_section = options.get('gst_section')
        #if options['date'].get('date_from') and options['date'].get('date_to'):
        filter_domain = [
        ...
        if gst_section:
            ...
            for gst_section, gst_section_name in self.get_gst_section(gst_return_type).items():
                ...
                lines.append({
                    ...
                    'action_id': self.env.ref('l10n_in_custom_reports.action_l10n_in_report_account').id,
    ...
    def get_gst_section_model_domain(self, gst_return_type, gst_section):
        ...
        # move_type changed into invoice_type
        if gst_return_type == 'gstr1':
            domain += [('journal_id.type', '=', 'sale')]
            if gst_section == 'b2b':
                domain += [
                    ('partner_vat', 'not in', (False, '')),
                ...
                domain += [
                    '&', '&', '&', ('partner_vat', 'in', (False, '')), ('journal_id.l10n_in_import_export', '!=', True), ('invoice_type', 'not in', ('out_refund', 'in_refund')),
