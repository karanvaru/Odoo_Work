    ...
    def _select(self):
        select_str = """SELECT aml.id AS id,
            ...
            CASE WHEN at.tax_group_id IN
                (SELECT res_id FROM ir_model_data WHERE module='l10n_in' AND name='sgst_group')
                THEN 0
                ELSE (CASE WHEN aml.tax_line_id IS NOT NULL THEN round(aml.tax_base_amount, 2) ELSE round(aml.balance, 2) * (CASE WHEN aj.type = 'sale' THEN -1 ELSE 1 END) END)
                END AS price_total,
            (CASE WHEN at.tax_group_id IN
                (SELECT res_id FROM ir_model_data WHERE module='l10n_in' AND name='sgst_group')
                THEN 0
                ELSE (CASE WHEN aml.tax_line_id IS NOT NULL THEN aml.tax_base_amount ELSE 1 END)
                END) + (round(aml.balance, 2) * (CASE WHEN aj.type = 'sale' THEN -1 ELSE 1 END))  AS total
        """
        return select_str
