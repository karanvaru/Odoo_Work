...
class VanillaOne(models.Model):
    _name = "vanilla.one"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = " Vanilla One"
    ...
    tag_ids = fields.Many2many('vanilla.one.tags', 'vanilla_one_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('vanilla.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('vanilla.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('vanilla.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('one.vanilla.sequence')
        res = super(VanillaOne, self).create(vals)
        return res
...
class VanillaOneTags(models.Model):
    _name = "vanilla.one.tags"

    _description = "Vanilla One Tags"

    name = fields.Char('Name')

class VanillaOnetoManyRelation(models.Model):
    _name = "vanilla.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('vanilla.one', string='relation')


class VanillaManyToManyRelation(models.Model):
    _name = "vanilla.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class VanillaManyToOneRelation(models.Model):
    _name = "vanilla.many.to.one.relation"
