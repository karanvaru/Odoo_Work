...
class Helpdesk_ticket_portal(CustomerPortal):
    ...
    def my_helpdesk_tickets(self, page=1, date_begin=None, date_end=None, filterby=None, groupby='none', sortby=None,
        ...
        searchbar_filters = {
            'open': {'label': _('Open'), 'domain': [('stage_id.is_close', '!=', True)]},
            'all': {'label': _('All'), 'domain': []},
            'closed': {'label': _('Closed'), 'domain': [('stage_id.is_close', '=', True)]},
        }

        searchbar_groupby = {
            'none': {'input': 'none', 'label': _('None')},
            'stage': {'input': 'stage', 'label': _('Stage')},
            'model_name': {'input': 'model_name', 'label': _('Model Name')},
        ...
        all_tickets = request.env['helpdesk.ticket'].search(custom_domain)

        if 'stage_id' in kw:
            if int(kw['stage_id']) != 0:
                domain += [('stage_id', '=', int(kw['stage_id']))]
            else:
                domain += [('stage_id', '!=', False)]

        def call_search_method(domain=[]):
            return request.env['helpdesk.ticket'].search(domain, order=order, limit=self._items_per_page,
                                                         offset=(page - 1) * self._items_per_page)

        tasks = request.env['helpdesk.ticket'].search(domain, order=order, limit=self._items_per_page,
                                                      offset=(page - 1) * self._items_per_page)

        lead_sudo = request.env['helpdesk.ticket'].sudo()

        if groupby == 'stage':
            grouped_tasks = [
                [request.env['helpdesk.ticket'].concat(*g)[0].stage_id.name if request.env['helpdesk.ticket'].concat(
                    *g) else '',
                 request.env['helpdesk.ticket'].concat(*g)] for k, g in groupbyelem(tasks, itemgetter('stage_id'))]
        # elif groupby == 'model_name':
        #     grouped_tasks = [
        #         [request.env['helpdesk.ticket'].concat(*g)[0].x_studio_model_name if request.env[
        #             'helpdesk.ticket'].concat(
        #             *g) else '',
        #          request.env['helpdesk.ticket'].concat(*g)] for k, g in
        #         groupbyelem(tasks, itemgetter('x_studio_model_name'))]
        ...
        # pager
        #         tickets_count = request.env['helpdesk.ticket'].search_count([('user_id', '=', request.env.user.id)])
        ...
        pager = portal_pager(
            ...
            url_args={'date_begin': date_begin, 'date_end': date_end, 'sortby': sortby, 'groupby': groupby,
                      'filterby': filterby, 'stage_id': stage_id},
            total=tickets_count,
            page=page,
        ...
        stage_tickets = request.env['helpdesk.ticket'].search(domain)

        request.session['my_tickets_history'] = tickets.ids[:100]
        filtered_stages = stage_tickets.mapped('stage_id')
        stage_ids = all_tickets.mapped('stage_id')

        stage_count_list = []
        for i in filtered_stages:
            stage_count_list.append({
                'id': i.id,
                'name': i.name,
                'count': len(all_tickets.filtered(lambda l: l.stage_id == i)),
                'url': '/my/tickets?stage_id=' + str(i.id) + '&' + 'groupby=' + groupby + '&' + 'filterby=' + filterby
            })

        stage_count_list.insert(0, {
            'id': 0,
            'name': "All",
            'count': len(all_tickets),
            'url': '/my/tickets?stage_id=' + str(0) + '&' + 'groupby=' + groupby + '&' + 'filterby=' + filterby
        })
        values.update({
            'date': date_begin,
            'tickets': tickets,
    ...
    def portal_ticket_create(self, contact_id=None, values={}, default_values={}, **kw):
        ...
        modal_name_dic = dict(request.env['helpdesk.ticket']._fields[''].selection)
        new_modal_list = [{
            'id': i,
            'name': k
    ...
    def portal_ticket_edit(self, ticket_id=None, **kw):
        ...
        if ticket_id:
            default_values.update({
                ...
                # 'x_studio_asp_ticket_no' : ticket_id.x_studio_asp_ticket_no,
                # 'x_studio_field_engineer_name' : ticket_id.x_studio_field_engineer_name,
                # 'x_studio_field_engineer_mobile' : ticket_id.x_studio_field_engineer_mobile,
            })
        if is_asc:
            teams = request.env['helpdesk.team'].sudo().search_read([('use_website_helpdesk_form', '=', True)],
                                                                    ['id', 'name'])
    ...
    def ticket_stage_change(self, ticket_id, stage_id=None, **kw):
        ticket_id = request.env['helpdesk.ticket'].browse(ticket_id)
        old_stage = ticket_id.stage_id.name
        url = '/'
        if ticket_id and stage_id:
            ticket_id.sudo().write({
                'stage_id': stage_id
            })
            new_stage = ticket_id.stage_id.name
            msg = _('Status Changed: %s to %s') % (old_stage, new_stage)
            ticket_id.sudo().message_post(body=msg, author_id=request.env.user.partner_id.id,
                                          message_type="comment",
    ...
    def ticket_helpdeskteam_validate(self, **kw):
        ...
        if team_id:
            ...
            if help_team_id:
                users_id = []
                members = help_team_id.member_ids or help_team_id.member_ids.search(
                    [('groups_id', 'in', request.env.ref('helpdesk.group_helpdesk_user').id)])
                for user in members:
