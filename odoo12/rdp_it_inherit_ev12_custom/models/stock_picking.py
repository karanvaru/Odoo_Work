    ...
    def _write(self, vals):
        ...
        for rec in self:
            if rec.move_ids_without_package:
                rec.move_ids_without_package.update({'record_type_id': rec.record_type_id.id})
                rec.move_ids_without_package.update({'record_category_id': rec.record_category_id.id})
        ...
        if self._context.get('invoice_sale_picking', False):
            return res

        if self._context.get('invoice_purchase_picking', False):
            return res
