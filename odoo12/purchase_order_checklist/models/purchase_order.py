...
STAGEVALIDATIONERRORMESSAGE = _(u"""Please enter check list for the purchase order '{0}'!
You can't change this purchase order state until you confirm all jobs have been done.""")
        ...
        if not self.env.user.has_group("purchase_order_checklist.group_purchase_order_checklist_superuser"):
            for order in self:
                ...
                if vals.get("state") not in no_needed_states:
                    ...
                    _logger.info("The length of check list line ids ", vals.get('check_list_line_ids'))
                    _logger.info("The length of check list line ids ", vals.get("check_list_line_ids"))
                    _logger.info("The length of check list line ids ", len(order.check_list_line_ids))
