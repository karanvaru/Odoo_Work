...
class VanillaNine(models.Model):
    _name = "vanilla.nine"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = " Vanilla Nine"
    ...
    tag_ids = fields.Many2many('nine.vanilla.one.tags', 'nine_vanilla_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('nine.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('nine.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('nine.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('nine.vanilla.sequence')
        res = super(VanillaNine, self).create(vals)
        return res
...
class NineVanillaOneTags(models.Model):
    _name = "nine.vanilla.one.tags"

    _description = "Vanilla Nine Tags"

    name = fields.Char('Name')

class NineOnetoManyRelation(models.Model):
    _name = "nine.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('nine.seven', string='relation')


class NineManyToManyRelation(models.Model):
    _name = "nine.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class NineManyToOneRelation(models.Model):
    _name = "nine.many.to.one.relation"
