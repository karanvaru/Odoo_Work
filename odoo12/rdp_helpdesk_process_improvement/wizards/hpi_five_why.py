...
class CreateFiveWhyResponse(models.TransientModel):
    _name = 'hpi.five.why.response'
    _description = 'Five Why Process Improvement'
    ...
    def action_five_why_submit(self):

        hpi_id = self.env['helpdesk.process.improvement']
        active_id = self._context.get('active_id')
        f_id = self.env['five.why'].browse(active_id)
        vals = {
            'created_by': self.created_by,
            'created_date': self.created_date,
            'subject': self.subject,
            'description': self.description,
            'five_why_ticket_id': f_id.id,

        }
        new_five_why_val = hpi_id.create(vals)
