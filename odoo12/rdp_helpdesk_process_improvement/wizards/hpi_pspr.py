...
class CreatePsprResponse(models.TransientModel):
    _name = 'hpi.product.details.response'
    _description = 'PSPR Process Improvement'
    ...
    def action_pspr_submit(self):

        hpi_id = self.env['helpdesk.process.improvement']
        active_id = self._context.get('active_id')
        pspr_id = self.env['product.details'].browse(active_id)
        vals = {
            'created_by': self.created_by,
            'created_date': self.created_date,
            'subject': self.subject,
            'description': self.description,
            'pspr_ticket_id': pspr_id.id,

        }
        new_pspr_val = hpi_id.create(vals)
