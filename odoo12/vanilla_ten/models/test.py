...
class VanillaTen(models.Model):
    _name = "vanilla.ten"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = " Vanilla Ten"
    ...
    tag_ids = fields.Many2many('ten.vanilla.one.tags', 'ten_vanilla_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('ten.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('ten.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('ten.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('ten.vanilla.sequence')
        res = super(VanillaTen, self).create(vals)
        return res
...
class TenVanillaOneTags(models.Model):
    _name = "ten.vanilla.one.tags"

    _description = "Vanilla Ten Tags"

    name = fields.Char('Name')

class TenOnetoManyRelation(models.Model):
    _name = "ten.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('ten.seven', string='relation')


class TenManyToManyRelation(models.Model):
    _name = "ten.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class TenManyToOneRelation(models.Model):
    _name = "ten.many.to.one.relation"
