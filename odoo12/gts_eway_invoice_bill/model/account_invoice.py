...
class AccountInvoice(models.Model):
    ...
    to_state = fields.Many2one("res.country.state", string='State', domain="[('country_id.code', '=', 'IN')]",
                               tracking=2)
    ...
    #         if (regex.search(self.invoice_no) == None):
    #             pass
    #         else:
    ...
    def onchage_ewabill_details(self):
        if self.generate_ewaybill:
            ...
            self.doc_date = self.date_invoice or False
            self.invoice_no = self.number

    @api.onchange('supply_type', 'sub_supply_type_id','generate_ewaybill')
    def onchange_generate_ewaybill(self):
        address = self.warehouse_id.partner_id #warehouse
        print('address', 'addressssssss')
        billing_address = self.partner_id #contact
        ...
        if address and billing_address and delivery_address and self.supply_type == 'O':
            self.from_name = self.warehouse_id.registered_name
            self.street = address.street
            self.street2 = address.street2
            self.city = address.city
            self.state_id = address.state_id.id
            ...
            if billing_address.parent_id:
                self.to_name = billing_address.parent_id.name
                print(self.to_name, 'billing_addresspartner_id.name')
            else:
                self.to_name = billing_address.name
                print(self.to_name, 'billing_address.name')
            # self.to_name = billing_address.name
            ...
            self.to_state = billing_address.state_id.id

        if address and billing_address and delivery_address and self.supply_type == 'I':
            # self.from_name = billing_address.name
            if billing_address.parent_id:
                self.from_name = billing_address.parent_id.name
                print(self.from_name, 'billing_addresspartner_id.name')
            else:
                self.from_name = billing_address.name
                print(self.from_name, 'billing_address.name')
            ...
            self.to_name = self.warehouse_id.registered_name
        ...
        if address and billing_address and self.supply_type == 'I' and sub_supply == 'Import':
            ...
            self.to_zip = address.zip
            self.to_vat = address.vat
            self.to_state = address.state_id.id
    ...
    def get_eway_bill_details(self):
        ...
        details_response = configuration.get_eway_details('GetEwayBill', self.ewaybill_no, self.warehouse_id)
    ...
    def print_eway_bill(self):
        ...
        attachment_data = {
            'name': 'EwayBill: ' + str(self.invoice_no or '') + ':' + str(self.name),
    ...
    def print_eway_bill_details(self):
        ...
        attachment_data = {
            'name': 'Detailed EwayBill: ' + str(self.invoice_no or '') + ':' + str(self.name),
            'datas': base64.b64encode(print_response),
            'type': 'binary',
            'res_model': 'account.invoice',
            'res_id': self.id,
    ...
    def generate_eway(self):
        ...
        if not self.warehouse_id:
            raise UserError(_('Warehouse is not set on Invoice %s !') % self.partner_id.name)
        if not self.warehouse_id.partner_id:
            raise UserError(_('Address is not set on Warehouse %s !') % self.warehouse_id.name)
        if not self.partner_id:
            raise UserError(_('Billing Address is not set %s !') % self.name)
        if not self.partner_id.street:
            raise UserError(_('Street is not set on Billing Address %s !') % self.partner_id.name)
        if not self.partner_id.street2:
            raise UserError(_('Street2 is not set on Billing Address %s !') % self.partner_id.name)
        if not self.partner_id.city:
            raise UserError(_('City is not set on Billing Address %s !') % self.partner_id.name)
        if not self.partner_id.state_id:
            raise UserError(_('State is not set on Billing Address %s !') % self.partner_id.name)
        if not self.partner_id.zip:
            raise UserError(_('Pincode is not set on Billing Address %s !') % self.partner_id.name)
        ...
        if not self.warehouse_id.gstin_live:
            raise UserError(_('GSTIN Live is not set on Warehouse %s !') % self.warehouse_id.name)
        if not self.warehouse_id.user_name_live:
            raise UserError(_('User Name is not set on Warehouse %s !') % self.warehouse_id.name)
        if not self.warehouse_id.ewb_password_live:
            raise UserError(_('Eway Bill Password Name is not set on Warehouse %s !') % self.warehouse_id.name)
        if not self.warehouse_id.registered_name:
            raise UserError(_('Registered Name is not set on Warehouse %s !') % self.warehouse_id.name)
        ...
        warehouse_details = self.warehouse_id
        amount_untaxed = 0
        ...
        order_dic = {
            ...
            'toStateCode': self.to_state.port_code,
        ...
        line_data = []
        total_cgst = total_igst = total_tcs = total_sgst = total_cess = total_cess_non_advol = 0.0
        if self.invoice_line_ids:
            for line in self.invoice_line_ids:
                if not line.uom_id.uom_mapping_id:
                    raise UserError(_('Eway Code is not set on UOM "%s" ') % (line.uom_id.name))
               # if not line.price_unit > 0:
                ...
                line_dic = {
                    ...
                    'qtyUnit': line.uom_id.uom_mapping_id.code,
                }
                cgst_rate = sgst_rate = igst_rate = tcs_rate = cess_rate = cess_non_279660908advol = 0.0
                if self.document_type in ('INV', 'BIL', 'BOE'):
                    product_price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
                    balance_taxes_res = line.invoice_line_tax_ids.compute_all(
                        product_price,
                    ...
                    for tax_dict in balance_taxes_res.get('taxes', []):
                        ...
                        elif 'TCS' in tax_dict.get('name') or tax.name.startswith('TCS'):
                            tcs_rate = tax.amount
                            total_tcs += tax_dict.get('amount', 0.0)
                ...
                line_dic.update({
                    ...
                    'cessNonAdvol':  0,
                    'taxableAmount': line.price_unit * line.quantity,
                })
        ...
        order_dic.update({
            ...
            'otherValue': total_tcs,
            'cessValue': total_cess,
            'cessNonAdvolValue': total_cess_non_advol,
            'totInvValue': amount_untaxed + total_cgst + total_sgst + total_igst + total_tcs + total_cess + total_cess_non_advol,
        })
        print('amount_untaxed', amount_untaxed, 'total_cgst', total_cgst, 'total_tcs', total_tcs, 'total_sgst', total_sgst , 'total_igst' , total_igst , 'total_cess', total_cess ,'total_cess_non_advol', total_cess_non_advol)
        ...
        response = configuration.generate_eway(self.warehouse_id, data_base64, 'GENEWAYBILL')
