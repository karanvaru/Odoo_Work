...
class TestPrintNodeReport(common.TransactionCase):
    ...
    def test_printnode_module_disabled(self):
        ...
        with self.assertRaises(UserError), self.cr.savepoint():
            self.printer.sudo(user=self.user).printnode_check_and_raise()
    ...
    def test_printnode_recheck(self):
        ...
        with self.assertRaises(UserError), self.cr.savepoint(), \
                patch.object(self.account, 'recheck_printer', return_value=False):
            self.printer.sudo(user=self.user).printnode_check_and_raise()
    ...
    def test_printnode_policy_report_no_size_and_printer_no_size(self):
        ...
        self.printer.sudo(user=self.user).printnode_check_report(self.report)
    ...
    def test_get_printer_for_action_button(self):
        ...
        printer, printer_bin = self.action_button.sudo(user=self.user)._get_action_printer()
        ...
        # Expecter UserError if there no one printer is available
        self.company.write({'printnode_printer': False})
        with self.assertRaises(UserError):
            self.action_button.sudo(user=self.user)._get_action_printer()
