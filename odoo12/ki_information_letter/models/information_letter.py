...
class InformationLetter(models.Model):
    _name = 'information.letter'
    _description = 'Information Letter'
    _inherit = ['mail.thread', 'website.seo.metadata', 'website.multi.mixin']
    _order = 'name'

    name = fields.Char('Information Letter Name', required=True, translate=True)
    subtitle = fields.Char('Blog Subtitle', translate=True)
    active = fields.Boolean('Active', default=True)

    @api.multi
    def write(self, vals):
        res = super(InformationLetter, self).write(vals)
        if 'active' in vals:
            # archiving/unarchiving a blog does it on its posts, too
            post_ids = self.env['information.letter.post'].with_context(active_test=False).search([
                ('blog_id', 'in', self.ids)
            ])
    ...
    def message_post(self, parent_id=False, subtype=None, **kwargs):
        ...
        if parent_id:
            ...
            if parent_message.subtype_id and parent_message.subtype_id == self.env.ref('ki_information_letter.mt_blog_blog_published'):
                if kwargs.get('subtype_id'):
                    kwargs['subtype_id'] = False
                subtype = 'mail.mt_note'
        return super(InformationLetter, self).message_post(parent_id=parent_id, subtype=subtype, **kwargs)
    ...
    def all_tags(self, min_limit=1):
        ...
        BlogTag = self.env['information.letter.tag']
        for blog_id in tag_by_blog:
            tag_by_blog[blog_id] = BlogTag.browse(tag_by_blog[blog_id])
        return tag_by_blog


class InformationLetterTagCategory(models.Model):
    _name = 'information.letter.tag.category'
    _description = 'Information Letter Tag Category'
    _order = 'name'

    name = fields.Char('Name', required=True, translate=True)
    tag_ids = fields.One2many('information.letter.tag', 'category_id', string='Tags')
...
class InformationLetterTag(models.Model):
    _name = 'information.letter.tag'
    _description = 'Information Letter Tag'
    _inherit = ['website.seo.metadata']
    _order = 'name'

    name = fields.Char('Name', required=True, translate=True)
    category_id = fields.Many2one('information.letter.tag.category', 'Category', index=True)
    post_ids = fields.Many2many('information.letter.post', string='Posts')
...
class InformationLetterPost(models.Model):
    _name = "information.letter.post"
    _description = "Information Letter Post"
    ...
    def _compute_website_url(self):
        super(InformationLetterPost, self)._compute_website_url()
        for blog_post in self:
            blog_post.website_url = "/information_letter/%s/post/%s" % (slug(blog_post.blog_id), slug(blog_post))
    ...
    blog_id = fields.Many2one('information.letter', 'Blog', required=True, ondelete='cascade')
    tag_ids = fields.Many2many('information.letter.tag', string='Tags')
    ...
    campaign_date = fields.Date(string="Campaign Date")
        ...
        if vals.get('website_published'):
            for post in self:
                post.blog_id.message_post_with_view(
                    'information.letter.letter_post_template_new_post',
                    subject=post.name,
                    values={'post': post},
    ...
    def create(self, vals):
        post_id = super(InformationLetterPost, self.with_context(mail_create_nolog=True)).create(vals)
    ...
    def write(self, vals):
        ...
        for post in self:
            ...
            result &= super(InformationLetterPost, self).write(copy_vals)
    ...
    def get_access_action(self, access_uid=None):
        ...
        if user.share and not self.sudo().website_published:
            return super(InformationLetterPost, self).get_access_action(access_uid)
        return {
    ...
    def _notify_get_groups(self, message, groups):
        ...
        groups = super(InformationLetterPost, self)._notify_get_groups(message, groups)
    ...
    def _default_website_meta(self):
        res = super(InformationLetterPost, self)._default_website_meta()
