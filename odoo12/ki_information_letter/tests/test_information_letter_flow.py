...
class TestWebsiteBlogFlow(TestWebsiteBlogCommon):
    ...
    def test_website_blog_followers(self):
        ...
        test_blog = self.env['information.letter'].sudo(self.user_blogmanager).create({
            'name': 'New Blog',
        })
        self.assertIn(
        ...
        test_blog_post = self.env['information.letter.post'].sudo(self.user_blogmanager).create({
            'name': 'New Post',
