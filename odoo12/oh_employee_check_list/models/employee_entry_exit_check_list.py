...
class EmployeeEntryDocuments(models.Model):
    ...
    def name_get(self):
        ...
        for each in self:
            if each.document_type == 'entry':
                ...
                name = each.name + '_ex'
            elif each.document_type == 'induction':
                name = each.name + '_ind'
            elif each.document_type == 'ojt':
                name = each.name + '_ojt'
            elif each.document_type == 'probation':
                name = each.name + '_probation'
            elif each.document_type == 'employment':
                name = each.name + '_emp'
            elif each.document_type == 'pre_exit':
                name = each.name + '_pre_ex'
            elif each.document_type == 'post_exit':
                name = each.name + '_post_ex'
    ...
    document_type = fields.Selection([('entry', 'Entry Process'),
                                      ...
                                      ('ojt', 'OJT Process'),
                                      ('probation', 'Probation Process'),
                                      ('employment', 'Employment Process'),
                                      ('pre_exit', 'Pre-Exit Process'),
                                      ('post_exit', 'Post Exit Process'),
