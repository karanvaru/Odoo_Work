...
class stock_picking(models.Model):
    ...
    def get_set_product(self,move,product):
        try :
            ...
            bom,lines = bom_point.explode(move.product_id,factor,picking_type=bom_point.picking_type_id)           
            return lines
        except:
            return {}
    ...
    datas = {'product_id':{'product_qty':qty,'traking_no':traking_no},'product_id':{'product_qty':qty,'traking_no':traking_no}} 
    datas = {'10':{'product_qty':1,'traking_no':784512369547},'12':{'product_qty':2,'traking_no':784512369548}}
    allow_extra_move if no one moves find related picking and product which you have pass in parameter..
            ...
            if not is_kit_product:
                ...
                for move in move_lines:
                    ...
                    for operation in operations:
                        ...
                        qty_left=float_round(float(qty_left) -op_qty,precision_rounding=operation.product_uom_id.rounding,rounding_method='UP')
                ...
                for product_id,bom_qty in transfer_product_qty.items():
                    ...
                    if float(qty_left)>0.0 and float(op_qty):
                        stock_move_line_obj.create(
                            {       
                                    'product_id': product_move_lines and product_move_lines[0].product_id.id,
                                    'product_uom_id':product_move_lines and product_move_lines[0].product_id.uom_id.id, 
                                    'picking_id':picking_id,
                                    'qty_done':float(qty_left) or 0,
                                    'result_package_id':package and package.id or False,
                                    'location_id':picking_obj.location_id.id, 
    ...
    """
    @api.multi
    def process_delivery_order_ept(self,picking_id,datas,traking_no=False,allow_extra_move=False):
        move_obj=self.env['stock.move']
        stock_move_line_obj=self.env['stock.move.line']
        pick_ids = []
        picking_obj = self.browse(picking_id)
    ...
    picking_id,source_location_id,location_dest_id,datas={'product_id':qty,'product_id':qty,'product_id':qty}
