...
class VanillaSix(models.Model):
    _name = "vanilla.six"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Vanilla Six"
    ...
    tag_ids = fields.Many2many('six.vanilla.one.tags', 'six_vanilla_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('six.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('six.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('six.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('six.vanilla.sequence')
        res = super(VanillaSix, self).create(vals)
        return res
...
class SixVanillaOneTags(models.Model):
    _name = "six.vanilla.one.tags"

    _description = "Vanilla six Tags"

    name = fields.Char('Name')

class SixOnetoManyRelation(models.Model):
    _name = "six.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('vanilla.six', string='relation')


class SixManyToManyRelation(models.Model):
    _name = "six.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class SixManyToOneRelation(models.Model):
    _name = "six.many.to.one.relation"
