...
class VanillaThree(models.Model):
    _name = "vanilla.three"
    ...
    tag_ids = fields.Many2many('three.vanilla.one.tags', 'vanilla_three_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('three.vanilla.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('three.vanilla.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('three.vanilla.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('three.vanilla.sequence')
        res = super(VanillaThree, self).create(vals)
        return res
...
class ThreeVanillaOneTags(models.Model):
    _name = "three.vanilla.one.tags"

    _description = "Vanilla Three Tags"

    name = fields.Char('Name')

class ThreeVanillaOnetoManyRelation(models.Model):
    _name = "three.vanilla.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('vanilla.Three', string='relation')


class ThreeVanillaManyToManyRelation(models.Model):
    _name = "three.vanilla.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class ThreeVanillaManyToOneRelation(models.Model):
    _name = "three.vanilla.many.to.one.relation"
