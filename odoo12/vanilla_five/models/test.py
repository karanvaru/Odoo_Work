...
class VanillaFive(models.Model):
    _name = "vanilla.five"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = " Vanilla Five"
    ...
    tag_ids = fields.Many2many('five.vanilla.one.tags', 'five_vanilla_tags_rel', 'name', string='Tags', track_visibility='onchange')
    ...
    one_to_many_ids = fields.One2many('five.one.to.many.relation', 'relation_id', string='One to Many')
    many_to_one_id = fields.Many2one('five.many.to.one.relation', string='Many to One')
    many_to_many_ids = fields.Many2many('five.many.to.many.relation',  string='Many to Many')
    # @api.model
    ...
    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('five.vanilla.sequence')
        res = super(VanillaFive, self).create(vals)
        return res
...
class FiveVanillaOneTags(models.Model):
    _name = "five.vanilla.one.tags"

    _description = "Vanilla Five Tags"

    name = fields.Char('Name')

class FiveOnetoManyRelation(models.Model):
    _name = "five.one.to.many.relation"

    _description = "One To Many"

    name = fields.Char('Name')
    relation_id = fields.Many2one('vanilla.five', string='relation')


class FiveManyToManyRelation(models.Model):
    _name = "five.many.to.many.relation"

    _description = "Many To Many"

    name = fields.Char('Name')

class FiveManyToOneRelation(models.Model):
    _name = "five.many.to.one.relation"
