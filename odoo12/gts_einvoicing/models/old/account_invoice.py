...
class account_invoice(models.Model):
    ...
    def create_einvoicing(self):
        ...
        _logger.info("sellar details============>>>>>>>>>>>>>>>%s>>>", SellerDtls)
        if self.partner_id.country_id.code != 'IN':
            ...
            _logger.info("Buyerdetails============>>>>>>>>>>>>>>>%s>>>", BuyerDtls)
        else:
            ...
            BuyerDtls = {
                ...
                "Pin": int(self.partner_id.zip),
                "Stcd": self.partner_id.state_id.stcd, }
            _logger.info("elseBuyer============>>>>>>>>>>>>>>>%s>>>", BuyerDtls)
        ...
        for idx, inv_line in enumerate(self.invoice_line_ids):
            if inv_line:
                    ...
                    for tax in inv_line.invoice_line_tax_ids:
                        ...
                        if tax.amount_type != 'group':
                            if tax.tax_group_id.name=="IGST" and tax.price_include==False:
                                total_igst = ((inv_line.price_unit*inv_line.quantity)-(inv_line.price_unit * inv_line.quantity)*inv_line.discount/100)*tax.amount/100
                                total_igsts +=((inv_line.price_unit*inv_line.quantity)-(inv_line.price_unit * inv_line.quantity)*inv_line.discount/100)*tax.amount/100
                                assmt = round(inv_line.price_subtotal - ((inv_line.price_unit * inv_line.quantity) - inv_line.price_subtotal),2)
                            elif tax.tax_group_id.name=="IGST" and tax.price_include==True:
                                total_igst=((inv_line.price_subtotal*inv_line.quantity)-(inv_line.price_unit * inv_line.quantity)*inv_line.discount/100)*tax.amount/100
                                total_igsts +=((inv_line.price_subtotal*inv_line.quantity)-(inv_line.price_unit * inv_line.quantity)*inv_line.discount/100)*tax.amount/100
                                assmt = inv_line.price_subtotal
                ...
                item_dict = {
                    ...
                    "Discount": discount,
                    # "AssAmt": assmt,
                    "AssAmt": round((inv_line.price_unit * inv_line.quantity) - discount, 2),
