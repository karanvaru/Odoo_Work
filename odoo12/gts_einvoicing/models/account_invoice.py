...
class account_invoice(models.Model):
    ...
    def create_einvoicing(self):
        ...
        flag = False
        _logger.info("======Dayan=======342===1========assmt=%s=",assmt)
        _logger.info("======Dayan=======342===1========total_amt=%s=",total_amt)
        for idx, inv_line in enumerate(self.invoice_line_ids):
            if inv_line:
                ...
                if inv_line.discount:
                    ...
                    _logger.info("======Dayan=======342===1========discount=%s=",discount)
                tax_rate = 0
        tax_rate = 0.0
        ...
        #             total_igsts += line.amount
        #         elif tax.tax_group_id.name == 'SGST':
        #             total_sgsts += line.amount
        #         elif tax.tax_group_id.name == 'CGST':
        #             total_cgsts += line.amount
        ...
        for inv_line in self.invoice_line_ids:
            ...
            if any('GST' in i['name'] for i in taxes):
                line_tax_rate = 0
                line_total_cgst = 0.0
                line_total_sgst = 0.0
                line_total_igst = 0.0
                for tax in taxes:
                    tax_id = self.env['account.tax'].sudo().browse(tax['id'])
                    if tax_id.tax_group_id.name == 'SGST':
                        line_total_sgst += tax['amount']
                        total_sgsts += tax['amount']
                    elif tax_id.tax_group_id.name == 'CGST':
                        line_total_cgst += tax['amount']
                        total_cgsts += tax['amount']
                    elif tax_id.tax_group_id.name == 'IGST':
                        line_total_igst += tax['amount']
                        total_igsts += tax['amount']

                    assmt = round(inv_line.price_subtotal, 2)
                    discount = (inv_line.price_unit * inv_line.quantity) * inv_line.discount / 100

                    line_tax_rate += tax_id.amount
                item_dict = {
                    "SlNo": str(idx + 1),
                    ...
                    "Discount": discount,
                    "AssAmt": assmt,
                    "SgstAmt": round(line_total_sgst, 2),
                    "CgstAmt": round(line_total_cgst, 2),
                    "IgstAmt": round(line_total_igst, 2),
                    "GstRt": line_tax_rate,
                    "TotItemVal": round(inv_line.price_total ,2)}
