...
class HelpdeskTeam(models.Model):
    ...
    ecom_todo_ticket_count = fields.Integer(
        string="Number of tickets", compute="_compute_todo_tickets_ecom"
    )
    ecom_todo_ticket_count_unassigned = fields.Integer(
        string="Number of tickets unassigned", compute="_compute_todo_tickets_ecom"
    )
    ecom_todo_ticket_count_unattended = fields.Integer(
        string="Number of tickets unattended", compute="_compute_todo_tickets_ecom"
    )
    ecom_todo_ticket_count_high_priority = fields.Integer(
        string="Number of tickets in high priority", compute="_compute_todo_tickets_ecom"
    )
    ...
    def _compute_todo_tickets(self):
        ...
        fetch_data = ticket_model.read_group(
            [("team_id", "in", self.ids), ("closed", "=", False), ('is_ecom_tickets', '=', False)],
    ...
    @api.depends("ticket_ids", "ticket_ids.stage_id")
    def _compute_todo_tickets_ecom(self):
        ticket_model = self.env["helpdesk.ticket"]
        fetch_data = ticket_model.read_group(
            [("team_id", "in", self.ids), ("closed", "=", False), ('is_ecom_tickets', '=', True)],
            ["team_id", "user_id", "unattended", "priority"],
            ["team_id", "user_id", "unattended", "priority"],
            lazy=False,
        )
        ...
        for team in self:
            team.ecom_todo_ticket_count = sum(r[4] for r in result if r[0] == team.id)
            team.ecom_todo_ticket_count_unassigned = sum(
                r[4] for r in result if r[0] == team.id and not r[1]
            )
            team.ecom_todo_ticket_count_unattended = sum(
                r[4] for r in result if r[0] == team.id and r[2]
            )
            team.ecom_todo_ticket_count_high_priority = sum(
                r[4] for r in result if r[0] == team.id and r[3] == "3"
            )
