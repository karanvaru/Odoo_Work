...
class HelpdeskTicket(models.Model):
    ...
    def _compute_is_normal_user(self):
        logged_in_user = self.env['res.users'].browse(self._uid)
        for rec in self:
            if logged_in_user.has_group('helpdesk_mgmt.group_helpdesk_normal_user') and rec.is_ecom_tickets:
                rec.is_normal_user = True
            else:
                rec.is_normal_user = False
    ...
    module_name = fields.Selection([('Sales', 'Sales'),
                                    ('Purchase', 'Purchase'),
                                    ('CRM', 'CRM'),
    ...
    is_normal_user = fields.Boolean(compute='_compute_is_normal_user',
        default=False)
    is_ecom_tickets = fields.Boolean(string='Ecom Ticket')
    ...
    def action_confirm(self):
        self.write({"is_confirm": True})

    @api.onchange("category_id", "module_name")
    def _onchange_category_id(self):
        self.partner_id = self.env.user.partner_id and self.env.user.partner_id.id or False
