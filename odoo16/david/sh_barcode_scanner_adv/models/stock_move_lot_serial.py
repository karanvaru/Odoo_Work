...
class StockMove(models.Model):
    ...
    def sh_stock_move_barcode_mobile_has_tracking(self, CODE_SOUND_SUCCESS,
                                                  CODE_SOUND_FAIL):
                    ...
                    for line in lines:
                        ...
                        if self.env.company.sudo(
                            ...
                            message = _(CODE_SOUND_SUCCESS +
                                        'Product: %s Qty: %s lot/serial: %s'
                                        ) % (self.product_id.name,
                                             line.qty_done, barcode)
            ...
            if self.product_id.tracking == 'lot':
                ...
                if not quant:
                    ...
                    if self.env.company.sudo().sh_stock_bm_is_notify_on_fail:
                        message = _(
                            CODE_SOUND_FAIL +
                            'There are no available qty for this lot: %s') % (
                                barcode)
                        self.env['bus.bus']._sendone(
                            self.env.user.partner_id,
                            'sh_inventory_barcode_mobile_notification_danger',
                        ...
                        if self.env.company.sudo(
                            ...
                            message = _(CODE_SOUND_SUCCESS +
                                        'Product: %s Qty: %s lot/serial: %s'
                                             ...
                                             line.qty_done, quant.lot_id.name)
                            self.env['bus.bus']._sendone(
                                self.env.user.partner_id,
                                'sh_inventory_barcode_mobile_notification_info',
                                {
            ...
            if self.product_id.tracking == 'serial':
                ...
                if lines:
                    for line in lines:
                        ...
                        if float_compare(line.qty_done,
                                         1.0,
                    ...
                    if not quant:
                        ...
                        if self.env.company.sudo(
                            ...
                            message = _(
                                CODE_SOUND_FAIL +
                                'There are no available qty for this lot/serial: %s'
                            ) % (barcode)
                            self.env['bus.bus']._sendone(
                                self.env.user.partner_id,
                                'sh_inventory_barcode_mobile_notification_danger',
    ...
    def sh_stock_move_barcode_mobile_no_tracking(self, CODE_SOUND_SUCCESS,
                                                 CODE_SOUND_FAIL):
        # move_lines = False
        #
        # # INCOMING
