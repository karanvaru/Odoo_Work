...
class SearchDocument(models.Model):
    ...
    def has_global_search_enabled(self):
        ...
        result = {
            'has_global_search_enabled': self.env.company.sh_global_document_search_is_enable,
            'options': options,
        }
        return result
    ...
    def _search_document_product(self, barcode):
        ...
        if data not in [None, False, '', ""]:
            ...
            if product and product.product_tmpl_id and product.product_tmpl_id.product_variant_ids and len(product.product_tmpl_id.product_variant_ids) == 1:
                action = self.env.ref(
                    'product.product_template_action').read()[0]
                action['domain'] = []
                action['views'] = [
...
#             result["action"] = action
    ...
    def _search_document_lot(self, barcode):
        ...
        if data not in [None, False, '', ""]:
            ...
            action['res_id'] = data[0]
            action["target"] = "new"
            # FIX in odoo15 in order to remove error in front error in js below
            # Error: NameError: name 'allowed_company_ids' is not defined
            action['context'] = {}
            result["action"] = action
...
#             action['context'] = {}
            action['domain'] = []
            action['views'] = [
                (self.env.ref('stock.view_location_form').id, 'form')]
            action['res_id'] = data[0]
#                 action["target"] = "current"
            action["target"] = "new"
            result["action"] = action
