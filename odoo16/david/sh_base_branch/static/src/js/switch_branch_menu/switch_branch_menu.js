...
export class SwitchBranchMenu extends Component {
    setup() {
        ...
        this.state = useState({ branchedToToggle: [] });
    }

    toggleBranch(branchId) {  
        browser.clearTimeout(this.toggleTimer);        
        this.BranchService.setBranches("toggle", branchId);        
    }

    logIntoBranch(branchId) {
        browser.clearTimeout(this.toggleTimer);
        this.BranchService.setBranch("loginto", branchId);
    }

    get selectedBranches() {
        return symmetricalDifference(
            this.BranchService.allowedBranchIds,
            this.state.branchedToToggle
        );
    }
}
...
registry.category("systray").add("SwitchBranchMenu", systrayItem, { sequence: 500 });
