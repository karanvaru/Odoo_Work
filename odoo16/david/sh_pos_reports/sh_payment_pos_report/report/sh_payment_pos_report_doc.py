...
class PaymentReport(models.AbstractModel):
    _name = 'report.sh_pos_reports.sh_payment_pos_report_doc'
                                        ...
                                        if invoice.move_type == "out_refund":
                                            ...
                                            if invoice_pay_dic.get(invoice.name, False):
                                                ...
                                                invoice_pay_dic.update({invoice.name: {journal.name: -1 * (journal_wise_payment.amount), "Total": -1 * (journal_wise_payment.amount), "Invoice": invoice.name,
                                                                                       "Customer": invoice.partner_id.name, "Customer_id": invoice.partner_id.id, "Invoice Date": invoice.invoice_date, "User": invoice.user_id.name if invoice.user_id else "","User_id": invoice.user_id.id if invoice.user_id else "",  "style": 'border: 1px solid black;color:red'}})
                                else:
                                    if not currency:
                                    ...
                                    for invoice in journal_wise_payment.pos_order_id.account_move:
                                        ...
                                        if invoice.move_type == "out_invoice":
                                            if invoice_pay_dic.get(invoice.name, False):
                                                ...
                                                invoice_pay_dic.update({invoice.name: {journal.name: journal_wise_payment.amount, "Total": journal_wise_payment.amount, "Invoice": invoice.name,
                                                                                       "Customer": invoice.partner_id.name, "Customer_id": invoice.partner_id.id,"Invoice Date": invoice.invoice_date, "User": invoice.user_id.name if invoice.user_id else "","User_id": invoice.user_id.id if invoice.user_id else "", "style": 'border: 1px solid black;'}})
                                        if invoice.move_type == "out_refund":
                                            j_refund += journal_wise_payment.amount
                                            if invoice_pay_dic.get(invoice.name, False):
                                                pay_dic = invoice_pay_dic.get(
