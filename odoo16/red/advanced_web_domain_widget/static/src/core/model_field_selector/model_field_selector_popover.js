...
export class ModelFieldSelectorPopoverBits extends Component {
  setup() {
    this.chain = Array.from(this.props.chain.slice(0, 1));
  ...
  async loadFields() {
    this.unfilteredFields = await this.modelField.loadModelFields(
      this.currentNode.resModel
    );
  ...
  onSearch(ev) {
    ...
    if (this.searchValue) {
      fieldKeys = fuzzyLookup(
        this.searchValue,
        fieldKeys,
        (key) => this.fields[key].string
      );
    }
...
ModelFieldSelectorPopoverBits.defaultProps = {
  validate: () => {},
...
ModelFieldSelectorPopoverBits.props = {
  chain: Array,
  update: Function,
...
ModelFieldSelectorPopoverBits.template =
  "advanced_web_domain_widget.ModelFieldSelectorPopoverBits";

export class ModelFieldSelectorPopoverBits2 extends ModelFieldSelectorPopoverBits {
  async loadFields() {
    this.unfilteredFields = await this.modelField.loadModelFields(
      this.chain[0].resModel
    );
    this.fields = { ...this.unfilteredFields };
    this.fieldKeys = this.sortedKeys(this.fields);
    for (let key of this.fieldKeys) {
...
ModelFieldSelectorPopoverBits2.template =
  "advanced_web_domain_widget.ModelFieldSelectorPopoverBits2";
