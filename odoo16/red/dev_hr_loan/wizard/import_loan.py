...
class import_loan(models.TransientModel):
    _name = "import.loan"
    _description = 'Model Import Loan'
    ...
    def get_employee_id(self,name):
        emp_id = self.env['hr.employee'].sudo().search([('name','=',name)],limit=1)
        if emp_id:
            return emp_id
        else:
            return False
        ...
        for line in lines:
            ...
            if not remark:
                ...
                if self.file_type == 'excel':
                    loan_date = datetime(*xlrd.xldate_as_tuple(line[4], workbook.datemode)).date()
                res ={
